...
- `/doctor` reports same-scope duplicate subagent `name` values and shows which definition is active (v2.1.196). Nested `.claude/agents/` directories: when more than one defines the same `name`, the definition closest to the working directory wins (v2.1.178).
