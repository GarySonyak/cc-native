...
`permissions.deny` rules now support glob patterns in the **tool-name position** (the part before the parenthesis). Example: `Web*` denies both `WebFetch` and `WebSearch` in a single rule. Previously, tool names in rules were exact-match only; globs only worked inside the argument specifier (e.g. `Bash(npm *)`). (v2.1.166) Claude Code now also warns at startup if a `Bash` **allow** rule places a wildcard before a subcommand argument (e.g. `Bash(npm * install)`), since that can match far more than intended. (v2.1.246)
...
Bash/PowerShell permission-check bypass fixes: PowerShell 5.1 permission-check bypass closed; Bash checks now fail closed on file-descriptor redirect forms parsed differently by bash vs. the permission analyzer; commands over **10,000 characters** now always prompt instead of silently auto-approving; zsh variable subscripts/modifiers inside `[[ ]]` comparisons (previously treated as inert text) now prompt; certain `help`/`man` invocations that could run unsafe options, command substitutions, or backslash paths no longer auto-approve. Remote-session permission prompts can no longer proceed before the local confirmation dialog answers. `docker`/Podman `docker`-shim commands carrying daemon-redirect flags (`--url`, `--connection`, `--identity`, Podman remote mode) now require a permission prompt (previously ran unprompted). Commands assigning an arithmetic expression to an integer shell variable (e.g. `OPTIND=1/0`, `RANDOM=2+2`) now also require approval instead of auto-approving; file tools (Read/Write/Edit) following a symlink swapped into the working directory after the permission check no longer read/write outside the approved location; Grep/Glob now apply `Read(...)` deny rules to files reached through a symlinked search path. (v2.1.251) zsh commands hiding a command substitution inside a `REPORTTIME`/`REPORTMEMORY`/`DIRSTACKSIZE` variable assignment no longer auto-approve; these now prompt too. (v2.1.260) **Reverted (v2.1.260)**: the v2.1.259 change extending Bash `Read()` deny rules to option/flag-value file arguments was rolled back — it denied plain commands like `npm run build` under a `Read(./**/build/**)` rule in every mode and forced `cd … && grep` to prompt even in auto mode; deny rules cover positional file arguments only again, as before v2.1.259.
