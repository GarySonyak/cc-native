...
- Bundled: `/batch`, `/claude-api`, `/debug`, `/loop`, `/code-review` (renamed from `/simplify` in v2.1.147; optional effort-level arg; `/simplify` alias retained temporarily). New in v2.1.145: `/run` (launch app to verify a change), `/verify` (confirm code change without tests), `/run-skill-generator` (record build/launch recipe so `/run`+`/verify` can follow it; run once per project).
