...
- **`/verify` and `/code-review` no longer self-trigger (v2.1.215)**: Claude will not invoke either skill on its own initiative anymore — only an explicit `/verify` or `/code-review` (from the user, or a skill/prompt that names it) runs them now. Don't rely on Claude auto-running a review or verification pass at the end of a task; ask for it explicitly.
...
- `/claude-api [migrate|upgrade|managed-agents-onboard]`: Load Claude API reference for Python/TypeScript/Java/Go/Ruby/C#/PHP/cURL + Managed Agents. **Auto-activates** when code imports `anthropic` or `@anthropic-ai/sdk`. `migrate` upgrades model IDs/thinking config in existing code to a target model; `managed-agents-onboard` walks through creating a new Managed Agent from scratch; `upgrade` migrates a Python project from the `anthropic` 0.x SDK to 1.x (timeouts now use `anthropic.Timeout`, not `httpx.Timeout`). (v2.1.239)
...
- `/claude-api` gained a `cost-optimize` subcommand: profiles Claude API spend and walks through cost-reduction levers; skill also expanded to cover Admin API operations (organization members/invites, workspaces, API keys, rate limits). (v2.1.247)
...
- A skill folder can hold its own `.claude-plugin/plugin.json`, which loads it as a plugin named `<name>@skills-dir` so it can also bundle hooks/agents/MCP servers; in a project's `.claude/skills/` this requires accepting the workspace-trust dialog first.
- In a **plugin** skill, frontmatter `name` replaces only the last segment of the invoked command — `my-plugin/skills/review/SKILL.md` with `name: fancy` becomes `/my-plugin:fancy` (bare `/fancy` also works unless another command already uses that name). Before v2.1.216, `name` replaced the whole command, so the menu showed bare `/fancy` with no plugin prefix and the namespaced form didn't autocomplete. For a plugin **root-level** `SKILL.md` (no `skills/` subdir), `name` supplies the whole final segment instead, falling back to the plugin's directory name if omitted. `claude plugin init <name>` scaffolds one directly. From v2.1.216 through v2.1.245, a `name` that already carried the plugin's own prefix (e.g. `name: my-plugin:fancy`) got the prefix doubled (`/my-plugin:my-plugin:fancy`); fixed to not double it as of v2.1.246.
