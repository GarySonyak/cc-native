...
- `enableArtifact`: user-level override to re-enable the Artifact tool for yourself when a higher (e.g. managed/project) scope set `disableArtifact`. `enableArtifact: false` or `disableArtifact: true` from *any* scope is honored even over a managed source setting the opposite — nothing turns the tool back on once either fires. Requires v2.1.242+.
