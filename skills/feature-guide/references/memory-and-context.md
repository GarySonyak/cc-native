...
Claude writes notes to `~/.claude/projects/<project>/memory/MEMORY.md` automatically. First 200 lines or 25KB loaded at session start. Topic files (e.g. `debugging.md`) not auto-loaded -- Claude reads on demand. Toggle: `autoMemoryEnabled` in settings or `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1`. Custom location: `autoMemoryDirectory` setting (must be absolute or `~/`-prefixed). `/memory` command to browse and edit. All worktrees in same git repo share one auto memory directory. Setting `CLAUDE_CODE_PROJECT_DIR_NAME` (beside `CLAUDE_CONFIG_DIR`, see settings.md) makes Claude Code use that name for the `<project>` directory instead of one derived per-repo — every repo launched with that config dir then shares one auto memory directory. (v2.1.234)
- Auto memory saves 4 kinds of notes, recorded as a `type` field in each memory file's frontmatter: `user` (role/expertise/preferences), `feedback` (corrections given/approaches confirmed), `project` (ongoing work/deadlines/decisions not derivable from code), `reference` (pointers to external systems). Claude skips anything derivable from the codebase or already stated in CLAUDE.md.
- `/memory`: GUI editors (e.g. VS Code) open the file in a separate window without blocking the session; before v2.1.216, `/memory` waited for the file to close before responding. Terminal editors (Vim) still take over the terminal until exit. (v2.1.216)
...
Sonnet 5's default auto-compact window widened to the full 1M-token context (was a smaller default) — compaction now triggers later in long sessions. (v2.1.247)
